...
use Classes\MareiObj as ClassesMareiObj;

use Drivers\PostgresDriver;
...
class DB{
    ...
    public $dbh = null, $table, $columns, $sql, $bindValues, $getSQL,
    $where, $orWhere, $whereCount=0, $isOrWhere = false,
    $rowCount=0, $limit, $orderBy, $lastIDInserted = 0, $joins = [], $joinsQuery;
    ...
    public function __construct()
    {
        ...
        try {
            $this->dbh = $driver->connect();
            $this->dbh->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_OBJ);
            
            if(env('db_transaction'))
                $this->beginTransaction();

        } catch (DB_Exception $e) {
            die(t("Error establishing a database connection.") . S . $e->getMessage());
        }
    ...
    {
        //assimble query
            $this->sql .= implode(' ', $this->joinsQuery) . $this->where;
    ...
    private function assimbleQuery()
    {
        ...
        $this->sql .= implode(' ', $this->joinsQuery) ?: '';
        if ($this->where !== null) {
            $this->sql .= $this->where;
        }
    ...
    public function count()
    {
        ...
        $countSQL .= implode(' ', $this->joinsQuery);
